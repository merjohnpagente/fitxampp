...
**Mode auto-detect:**
- `http://localhost/fitcore-gms/` → **MySQL mode** (green badge “● MySQL Connected”)
...
3. Click **Import** → Choose file → `C:\xampp\htdocs\fitcore-gms\sql\database.sql` → Go
4. Verify: left sidebar shows `fitcore_gym` with 12 tables (`users`, `members`, `plans`, etc.)

### D. Verify
1. Open **http://localhost/fitcore-gms/check.php**
   - All checks should be green. If red, check MySQL is running and `sql/database.sql` was imported.
2. Open **http://localhost/fitcore-gms/index.html** (or `http://localhost/fitcore-gms/`)
...
Force demo mode even on localhost: `http://localhost/fitcore-gms/index.html?demo=1`
...
| No “● MySQL Connected” badge | You opened via `file://` — use `http://localhost/fitcore-gms/` |
...
| 404 on `api/*.php` | Ensure folder is `C:/xampp/htdocs/fitcore-gms/` and Apache is running |
...
- **Project zip**: Zip the whole `fitcore-gms` folder (includes `index.html` + `api/` + `sql/database.sql`) — ready to submit.
